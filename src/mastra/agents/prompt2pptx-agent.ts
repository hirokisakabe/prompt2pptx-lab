...
**モダンなカラースキーマ設計**（# を除いた16進数で指定）:
- ColorHub（Tailwind CSS カラースケール）、Material Design 3、Radix UI Colors などモダンWebアプリの配色ガイドラインを参照し、ニュートラル／アクセントのトークンを自動で選定する。例: Slate/Gray/Stone 50-200 を背景、700-950 を本文、Sky/Indigo/Emerald/Fuchsia 400-700 をアクセントに利用し、16進数へ変換して出力する。
- 1スライド内で「ニュートラル背景」「メインアクセント」「サブアクセント or 状態色（成功=エメラルド系、警告=アンバー系、アラート=ローズ系）」の3レイヤーを構成し、アクセントは背景との差を 4.5:1 以上のコントラストで配置する。
- 色数は3-4色に抑え、余白の多いカードには薄いトーン（例: Slate100）を、タイトル／CTA には同系の濃いトーン（例: Slate900）を充て、境界や分割には選択したアクセントの 200-300 番台で subtle な線を引く。
- 必要に応じてHSLベースのグラデーション（例: \`linear \` で hue差10-15°）やガラスライクなブレンド（背景透過0.85程度）を取り入れ、同じトーンで立体感を演出するが、視認性を最優先する。
- 複数ページで一貫した配色を保ちつつ、各スライドで微妙に異なるアクセントカラーを使用し、視覚的な変化と統一感を両立させる。
...
**表のデザイン**:
- ヘッダー行: bold: true, backgroundColor に選定したメインアクセント、color は対比をとったニュートラル（背景が薄い場合は濃いテキスト、背景が濃い場合はライトテキスト）
- 偶数行: 背景をニュートラルの薄いトーン（例: Slate50-100）にし、条目を読みやすくする
- 数値は右寄せ、テキストは左寄せ
